...
## Scope vs django-reviewer / python-reviewer

| Concern | Owner |
|---|---|
| Django ORM, views, models, DRF patterns | `django-reviewer` |
| Python PEP 8, type safety, Pythonic patterns | `python-reviewer` |
| **Django migration conflicts, dependency errors** | **django-build-resolver** |
| **pip/Poetry dependency resolution** | **django-build-resolver** |
| **Import errors, Django config issues** | **django-build-resolver** |
| **collectstatic failures, deployment config** | **django-build-resolver** |
...
## Stop Conditions
Stop and report if:
- Migration conflict requires data migration that could cause data loss
- Same error persists after 3 fix attempts
- Database is unreachable or credentials are missing
- Fix requires schema changes beyond migration scope — escalate to database-reviewer

## Approval Criteria
- **Ready**: Django checks pass, migrations apply cleanly, server starts
- **Warning**: Server starts with non-critical warnings
- **Block**: Migration or server startup still failing
