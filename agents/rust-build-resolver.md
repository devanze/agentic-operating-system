---
description: Rust build, Cargo, and compilation error resolution specialist.
mode: subagent
...
## Scope vs rust-reviewer

| Concern | Owner |
|---|---|
| Ownership, lifetimes, idiomatic Rust | `rust-reviewer` |
| Concurrency patterns, unsafe code review | `rust-reviewer` |
| **Cargo build errors, dependency resolution** | **rust-build-resolver** |
| **Linker errors, native library issues** | **rust-build-resolver** |
| **Feature flag conflicts, edition migration** | **rust-build-resolver** |
| **clippy lint resolution, compiler upgrade issues** | **rust-build-resolver** |
...
## Approval Criteria
- **Ready**: `cargo build` and `cargo clippy` pass with zero errors
- **Warning**: Build passes with clippy warnings
- **Block**: Build or clippy still failing